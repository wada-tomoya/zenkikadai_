...
namespace dxe {
	...
	void Mesh::copySetting(Mesh* other) {
		other->vbo_ = vbo_;
	...
	//----------------------------------------------------------------------------------------
	Mesh* Mesh::CreateFromTypeDesc(dxe::Mesh::eShapeType shpe_type, dxe::MeshDesc* desc) noexcept {
		switch (shpe_type) {
	...
	std::vector<Mesh*> Mesh::CreateFromFileMVT(const std::string& file_path) {
